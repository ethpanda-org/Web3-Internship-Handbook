...
1.  ==**恆定乘積公式：x \* y = k**==
    ...
    - 這樣，Uniswap 就能自動算出兌換的價格，而且隨著池子中資產比例的變化，價格也會自動調整。若池內價格與外部市場偏離，會被套利交易逐步拉回。
      :::
      Uniswap 透過這個公式，自動決定每個交易的價格和兌換率。更準確地說，它讓報價由池子儲備比例連續決定；當單筆交易相對池子過大時，會出現明顯滑點。
...
2.  ==**流動性池和流動性提供者（LP）**==
    ...
    **那流動性提供者（LP）是如何賺錢的？**
    ...
    - 假設該 ETH/USDC 流動性池總儲備是 10 ETH 與 5000 USDC（約 500 USDC/ETH）。如果別人用 1 ETH 交換 USDC，這筆交易會產生 0.003 ETH 手續費（0.3%），有效輸入 0.997 ETH，對應輸出約 453.31 USDC。
    - **具體收益計算（簡化）**：
    ...
    - 註：此例只用於說明手續費如何分配。LP 真實收益仍會受無常損失、交易量、價格波動與 Gas 成本影響。
      :::
...
3.  ==**去中心化 vs. 中心化交易所的好處**==
    ...
    | **大額交易更便捷**           | 你不需要等待層層審批，交易由智能合約和流動性池完成。所有交易都直接在鏈上執行。 | 大額資金的交易通常需要透過銀行、金融機構等審批環節，可能還要經過反洗錢審查等複雜的程式，時間較長。 |
...
### 3. ==AI + DeFi：智慧化金融服務（仍在早期）=={.important}

**人工智慧與去中心化金融**的結合仍在持續探索。需要注意的是，很多項目屬於「數據建模 + 自動化執行」，不一定是嚴格意義上的深度學習系統。

::: steps

1. **智能策略與組合管理**

   - **Numerai**：透過眾包機器學習模型進行市場預測，屬於「AI + 量化」結合的代表案例。
   - **SingularityDAO**：嘗試用演算法驅動動態投資組合（Dynasets），自動調整資產權重。

2. **風險監測與異常檢測**

   - **Forta**：基於鏈上監測網路識別可疑交易與安全風險，部分場景使用機器學習方法。
   - **Chaos Labs / Gauntlet**：更偏向仿真建模與風險參數調優，協助協議做風控決策。

3. **AI 代理與交互體驗**

   - **Fetch.ai**：探索可自主執行任務的鏈上代理網路。
   - **自然語言交互工具**（持續演進中）：讓使用者用更低門檻方式完成查詢、策略執行與多步驟操作。

:::

### 4. ==RWA 資產上鏈：傳統資產的鏈上表達=={.important}

RWA（Real-World Assets）是把國債、基金份額、應收帳款等現實世界資產透過合規結構映射到鏈上。它的價值在於：為鏈上資金提供相對穩定、可驗證的收益來源，也讓傳統金融與鏈上金融產生連接。

**核心關注點：**

- **託管與贖回**：託管方是誰、贖回週期與費用是否透明
- **合規與司法轄區**：發行主體是否具備合規資質，投資者範圍是否清晰
- **持續披露**：是否提供可驗證的資產證明、審計與定期資訊披露

**常被提及的項目：** Ondo、Centrifuge、Maple

::: card
這些跨領域的創新不僅展示了 Web3 技術的可組合性，也為使用者創造了更豐富的應用場景。隨著技術的不斷發展，我們預計會看到更多領域的融合創新，如 Social + DeFi、IoT + Blockchain 等。
:::

### 5. ==WEB3 + 鄉建：南塘 DAO 的探索=={.important}
...
![2026 年新興趨勢](../images/industry/web3-tendency.png)
