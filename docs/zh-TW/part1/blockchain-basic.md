...
去中心化的網路由無數節點提供服務來維持網路執行。以 PoW 鏈為例，這些操作常被稱為「挖礦」，參與者一般被稱為礦工。礦工（或 PoS 鏈中的驗證者）維持網路執行需要獎勵，就像你工作需要工資。更準確地說，節點收入通常由**協議獎勵**與**交易手續費**兩部分組成。你經常聽說的燃料費（Gas Fee）屬於交易手續費的一部分，並不等於全部節點獎勵。
...
1. **使用者發起交易**：使用者透過錢包應用發起轉賬、智能合約呼叫等操作
...
6. **獎勵發放**：成功出塊的節點可獲得協議獎勵（如區塊獎勵/質押獎勵）和交易手續費（或其中一部分）  
<br>
...
4.  **開放創新生態**
    ...
    區塊鏈與智能合約構建了去中心化應用（Dapps）平臺，任何開發者都可在此基礎上創新並獲得代幣激勵，促進了技術和商業模式的多樣化。
