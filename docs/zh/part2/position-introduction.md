...
1. **主要职责：**
   ...
   - 审查智能合约逻辑、权限控制、资产流向与边界条件，识别高危漏洞。
   - 输出结构化审计报告（问题描述、风险等级、修复建议），并跟踪复测结果。
   - 结合真实攻击案例与链上数据，完善项目威胁模型与安全基线。

2. **职位要求：**

   - 熟悉 Solidity/EVM、常见攻击面（重入、权限绕过、预言机操纵、签名重放等）。
   - 能使用 Foundry、Slither、Mythril、Echidna 等工具进行静态/动态分析。
   - 具备 PoC 编写与安全沟通能力，能把技术风险转化为可执行修复方案。
