...
1.  ==**恒定乘积公式：x \* y = k**==
    ...
    - 这样，Uniswap 就能自动算出兑换的价格，而且随着池子中资产比例的变化，价格也会自动调整。若池内价格与外部市场偏离，会被套利交易逐步拉回。
      :::
      Uniswap 通过这个公式，自动决定每个交易的价格和兑换率。更准确地说，它让报价由池子储备比例连续决定；当单笔交易相对池子过大时，会出现明显滑点。
...
2.  ==**流动性池和流动性提供者（LP）**==
    ...
    **那流动性提供者（LP）是如何赚钱的？**
    ...
    - 假设该 ETH/USDC 流动性池总储备是 10 ETH 与 5000 USDC（约 500 USDC/ETH）。如果别人用 1 ETH 交换 USDC，那么这笔交易会产生 0.003 ETH 手续费（0.3%），有效输入 0.997 ETH，对应输出约 453.31 USDC。
    - **具体收益计算（简化）**：
    ...
    - 注：该示例只用于说明手续费如何分配。LP 真实收益还会受到无常损失、交易量、价格波动与 Gas 成本影响。
      :::
...
3.  ==**去中心化 vs. 中心化交易所的好处**==
    ...
    | **大额交易更便捷**           | 你不需要等待层层审批，交易由智能合约和流动性池完成。所有交易都直接在链上执行。 | 大额资金的交易通常需要通过银行、金融机构等审批环节，可能还要经过反洗钱审查等复杂的程序，时间较长。 |
...
### 3. ==AI + DeFi：智能化金融服务（仍在早期）=={.important}

**人工智能与去中心化金融**的结合正在持续探索中。需要注意的是，很多项目属于“数据建模 + 自动化执行”，不一定是严格意义上的深度学习系统。

::: steps

1. **智能策略与组合管理**

   - **Numerai**：通过众包机器学习模型进行市场预测，属于“AI + 量化”结合的代表案例。
   - **SingularityDAO**：尝试用算法驱动动态投资组合（Dynasets），自动调整资产权重。
...
3. **AI 代理与交互体验**
   ...
   - **Fetch.ai**：探索可自主执行任务的链上代理网络。
   - **自然语言交互工具**（持续演进中）：让用户用更低门槛方式完成查询、策略执行与多步骤操作。

:::

### 4. ==RWA 资产上链：传统资产的链上表达=={.important}
...
![2026 年新兴趋势](../images/industry/web3-tendency.png)
