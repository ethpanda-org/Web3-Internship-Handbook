...
1.  ==**恒定乘积公式：x \* y = k**==
    ...
    - 偏离了初始价格比例的池子存在套利空间。假设在池子以外，ETH 的市场价格是 500 USDC。现在，只需要往池子里存入 454.55 USDC 就能换回 1 个 ETH。套利者会迅速实施套利，从而把价格比例拉回市场公允水平。
      :::
      Uniswap 通过这个公式，自动决定每个交易的价格和兑换率。在恒定乘积公式 x \* y = k 的作用下，价格会跟随储备比例连续调整并趋近市场公允价值。大额单会产生滑点。
