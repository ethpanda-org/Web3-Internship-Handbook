...
1.  ==**恒定乘积公式：x \* y = k**==
    ...
    - 这样，Uniswap 就能自动算出兑换的价格，而且随着池子中资产比例的变化，价格也会自动调整。
      :::
      Uniswap 通过这个公式，自动决定每个交易的价格和兑换率。它确保报价由池子储备比例决定并连续调整；当交易规模相对池子较大时，会出现明显滑点。
...
2.  ==**流动性池和流动性提供者（LP）**==
    ...
    **那流动性提供者（LP）是如何赚钱的？**
    ...
    - 假设该 ETH/USDC 流动性池当前总储备为 10 ETH 和 5000 USDC（约 500 USDC/ETH）。如果有人用 1 ETH 兑换 USDC，按 0.3% 手续费计算，手续费为 0.003 ETH，有效输入为 0.997 ETH，在恒定乘积 AMM 下，本次交易输出约 453.31 USDC。
    - **具体收益计算（简化）**：
    ...
    - 注：该例仅展示“手续费如何产生与分配”。LP 的真实收益还会受到无常损失、交易量、价格波动、Gas 成本等影响。
      :::
...
3.  ==**去中心化 vs. 中心化交易所的好处**==
    ...
    | **大额交易更便捷**           | 你不需要等待层层审批，特别是对于一些大额资金的交易，完全依赖智能合约和流动性池来完成。所有交易都直接在链上进行，快速而高效。但大额交易可能滑点更高。 | 大额资金的交易通常需要通过银行、金融机构等审批环节，可能还要经过反洗钱审查等复杂的程序，时间较长。 |
